...
concerning the int[][] in use :

    the ball will be presented as 70, so it can stack with per example a block invicible, and it will resutl in an int value of 70+4=74

    concerning snoopy, we will simply add its int value to the block he is on top of
    using this method, we can ded